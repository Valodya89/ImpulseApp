...
//  MimoBike
//
//  Created by Albert Mnatsakanyan on 14.09.21.
//
...
// Example: MiAlertView().showSuccess(self, title: "Test", subTitle: "Value").close()
...
let uniqueAccessibilityIdentifier: String = "MiAlertView"

public typealias DismissBlock = () -> Void

// The Main Class
open class MiAlertView: UIViewController {
    ...
    fileprivate var selfReference: MiAlertView?
    ...
    fileprivate func setup() {
        ...
        if appearance.disableTapGesture == false {
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(MiAlertView.tapped(_:)))
    ...
    override open func viewWillLayoutSubviews() {
        ...
        let rv = UIApplication.shared.keyWindowInConnectedScenes! as UIWindow
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MiAlertView.keyboardWillShow(_:)), name:UIResponder.keyboardWillShowNotification, object: nil);
        NotificationCenter.default.addObserver(self, selector: #selector(MiAlertView.keyboardWillHide(_:)), name:UIResponder.keyboardWillHideNotification, object: nil);
    }
    ...
    open func addButton(_ title:String, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, showTimeout:SCLButton.ShowTimeoutConfiguration? = nil, action:@escaping ()->Void)->SCLButton {
        ...
        btn.action = action
        btn.addTarget(self, action:#selector(MiAlertView.buttonTapped(_:)), for:.touchUpInside)
        btn.addTarget(self, action:#selector(MiAlertView.buttonTapDown(_:)), for:[.touchDown, .touchDragEnter])
        btn.addTarget(self, action:#selector(MiAlertView.buttonRelease(_:)), for:[.touchUpInside, .touchUpOutside, .touchCancel, .touchDragOutside] )
        return btn
    }
    ...
    //Dismiss keyboard when tapped outside textfield & close MiAlertView when hideWhenBackgroundViewIsTapped
    ...
    open func showSuccess(_ title: String, subTitle: String? = nil, closeButtonTitle:String?=nil, timeout:SCLTimeoutConfiguration?=nil, colorStyle: UInt=SCLAlertViewStyle.success.defaultColorInt, colorTextButton: UInt=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        return showTitle(title, subTitle: subTitle, timeout: timeout, completeText:closeButtonTitle, showCloseButton: closeButtonTitle != nil, style: .success, colorStyle: colorStyle, colorTextButton: colorTextButton, circleIconImage: circleIconImage, animationStyle: animationStyle)
    }
    ...
    open func showError(_ title: String, subTitle: String? = nil, closeButtonTitle:String?=nil, timeout:SCLTimeoutConfiguration?=nil, colorStyle: UInt=SCLAlertViewStyle.error.defaultColorInt, colorTextButton: UInt=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        return showTitle(title, subTitle: subTitle, timeout: timeout, completeText:closeButtonTitle, showCloseButton: closeButtonTitle != nil, style: .error, colorStyle: colorStyle, colorTextButton: colorTextButton, circleIconImage: circleIconImage, animationStyle: animationStyle)
    }
    ...
    open func showInfo(_ title: String, subTitle: String? = nil, closeButtonTitle:String?=nil, showCloseButton: Bool = false, timeout:SCLTimeoutConfiguration?=nil, colorStyle: UInt=SCLAlertViewStyle.info.defaultColorInt, colorTextButton: UInt=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        return showTitle(title, subTitle: subTitle, timeout: timeout, completeText:closeButtonTitle, showCloseButton: showCloseButton, style: .info, colorStyle: colorStyle, colorTextButton: colorTextButton, circleIconImage: circleIconImage, animationStyle: animationStyle)
    }
    ...
    open func showTitle(_ title: String, subTitle: String? = nil, timeout: SCLTimeoutConfiguration?, completeText: String?, showCloseButton: Bool = false, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        ...
        if let rv = UIApplication.shared.topMostViewController()?.view  {
            rv.addSubview(view)
            ...
            // Done button
            if appearance.showCloseButton || showCloseButton {
                _ = addButton(completeText ?? "Done", target:self, selector:#selector(MiAlertView.hideView))
            }
            ...
            if let timeout = timeout {
                ...
                timeoutTimer = Timer.scheduledTimer(timeInterval: timeout.value, target: self, selector: #selector(MiAlertView.hideViewTimeout), userInfo: nil, repeats: false)
                showTimeoutTimer?.invalidate()
                showTimeoutTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(MiAlertView.updateShowTimeout), userInfo: nil, repeats: true)
            }
    ...
    @objc open func hideView() {
        UIView.animate(withDuration: 0.2, animations: {
            ...
            // This is necessary for MiAlertView to be de-initialized, preventing a strong reference cycle with the viewcontroller calling MiAlertView.
    ...
    //Return true if a MiAlertView is already being shown, false otherwise
    open func isShowing() -> Bool {
        if let subviews = UIApplication.shared.keyWindowInConnectedScenes?.subviews {
