...
protocol EVChargerWorkerProtocol {    
    ...
    func getChargingStationDetailed(id: String) -> AnyPublisher<(EVChargingStation, [EVStationFeedback]), MimoError>
...
class EVChargerWorker: EVChargerWorkerProtocol {
   ...
   private var chargerStateSubject = CurrentValueSubject<EVStateMessagedDTO?, Never>(nil)
    ...
    init(useCase: EVChargerUseCaseProtocol) {
...
//        }
    ...
    func getChargingStationDetailed(id: String) -> AnyPublisher<(EVChargingStation, [EVStationFeedback]), MimoError> {
        Deferred {
            Future<(EVChargingStation, [EVStationFeedback]), MimoError> { promise in
                self.evChargerRepository.getChargingStationDetailed(id: id) { result in
                    switch result {
                    case .success(let data):
                        let station = EVChargingStation(station: data.station, chargings: data.chargings)
                        let feedbacks = data.feedbacks.map { EVStationFeedback(feedback: $0) }
                        promise(.success((station, feedbacks)))
                    case .failure(let error):
                        promise(.failure(.init(error: error)))
                    }
                }
