...
struct EVChargerDetailsView: View {
    ...
    var selectedMediaOverlay: some View {
        if let selectedLogoIcon = viewModel.selectedLogoIconURL {
            overlayBackground {
                KFImage(selectedLogoIcon)
                    .resizable()
                    .scaledToFit()
                    .clipShape(RoundedRectangle(cornerRadius: 20))
                    .frame(width: viewModel.cardWidth, height: 300)
            }
...
//                    Spacer()
        ...
        HStack(alignment: .top, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 2) {
                    ...
                    Text("(6 " + "EV_CHARGER_reviews".localized() + ")")
    ...
    private func tabsView() -> some View {
        TabSegmentedControl(selected: $selectedPage, pages: [
            ("EV_CHARGER_connectors".localized(), AnyView(connectorList(connectors: viewModel.station?.connectors ?? []))),
    ...
    @ViewBuilder
    private func connectorList(connectors: [EVChargingConnector]) -> some View {
