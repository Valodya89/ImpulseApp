...
final class EVChargerDetailsViewModel: MimoBaseViewModel, ObservableObject {
    ...
    func loadData() {
        let stationPublisher: AnyPublisher<EVChargingStation, MimoError> = byStationId
            ? evChargerWorker.getChargingStationDetailedByStationId(stationId: stationId)
        ...
        } receiveValue: { [weak self] station, wallet, financialState, user, account in
