...
struct HomeScanQRViewModel {
    ...
    func getUserAccount(completion: @escaping (Result<UserResult, Error>) -> Void) {
        accountRepository.getUserAccount { (result) in
            switch result {
            case .success(let userResponse):
                let userResult = AccountMapper.toUserResult(from: userResponse)
